...
describe("integrate spine SSOT golden fixtures", () => {
  ...
  it("adoption_epistemics_contains_pinned_integratespine_row", () => {
    const doc = readFileSync(join(root, "docs", "adoption-epistemics.md"), "utf8");
    const pinned = readFileSync(join(root, "test", "fixtures", "integrate-spine-adoption-row-pinned.txt"), "utf8").trim();
    assert.ok(doc.includes(pinned), "adoption-epistemics.md must contain pinned four-way table row");
  });
});
