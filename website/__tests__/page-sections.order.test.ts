...
describe("HOME_SECTION_ORDER", () => {
  it("matches conversion funnel order (eight sections)", () => {
    ...
    expect(HOME_SECTION_ORDER.length).toBe(8);
  });
  ...
  it("orders hero through commercialSurface monotonically", () => {
    ...
    const ev = HOME_SECTION_ORDER.indexOf("evaluatorTruthAndAdoption");
    ...
    expect(st).toBeLessThan(ev);
    expect(ev).toBeLessThan(hw);
