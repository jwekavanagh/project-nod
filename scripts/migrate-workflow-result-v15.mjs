/**
 * Upgrade saved WorkflowResult JSON from schemaVersion 14 → 15 by recomputing workflowTruthReport
 * (truth schemaVersion 8 + correctnessDefinition).
 *
...
function shouldMigrateFile(fp) {
  ...
  if (rel.includes("/examples/debug-corpus/") && !rel.includes("/examples/debug-corpus/run_ok/")) return false;
  return true;
}
...
function needsBump(j) {
  return j && typeof j === "object" && j.schemaVersion === 14 && Array.isArray(j.steps);
}
...
if (migrated === 0) {
  console.log("migrate-workflow-result-v15: no schemaVersion 14 workflow-result-shaped files found");
}
