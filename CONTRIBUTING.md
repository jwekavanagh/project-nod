...
- **IDE-style messages:** short summaries without a `type:` prefix (for example from Cursor) are **allowed**. If a line looks like a Conventional Commit (`chore: …`, `feat(scope): …`, etc.), the **[@commitlint/config-conventional](https://github.com/conventional-changelog/commitlint/tree/master/%40commitlint/config-conventional)** rules apply with one local override: body line length is not enforced (`body-max-line-length` disabled) so IDE-generated bullet lists are not blocked. Otherwise the hook does not block the commit. For **versioning and changelogs** on `main`, semantic-release still only recognizes conventional commit **types**—use a conventional first line for commits that should drive releases, or **squash-merge** the PR and set the squash title to a conventional message.
...
| `replay_verification_truth` | `Replay verification truth (Docker)` | `CI / Replay verification truth (Docker)` |
...
Verify with `gh pr checks <PR> --json name,state` on a green PR: the PR-visible job `name` values in the table must match character-for-character.
...
- **When:** on every **push to `main`** that completes [**`CI`**](.github/workflows/ci.yml) successfully (excluding follow-up completions whose run title begins with **`chore(release):`**), [**`release.yml`**](.github/workflows/release.yml) runs **semantic-release** via `workflow_run` on that same merge commit when semantic-release determines a new version is needed (releasable Conventional Commit since the last [`v*.*.*`](https://github.com/semver/semver) tag). **`CI`** intentionally runs **light verification**—not the full Postgres/Docker/replay/Python suite—for **`chore(release):` pushes to `main`** and **`v*` tag pushes**, while release-generated tags ship the wheel via [**`release-package.yml`**](.github/workflows/release-package.yml).
...
- **PyPI:** when semantic-release **pushes** the `vX.Y.Z` tag, [**`release-package.yml`**](.github/workflows/release-package.yml) runs on **`push`** to that tag only, builds the wheel from [`python/`](python/), publishes to PyPI with **Trusted Publishing (OIDC)** via `pypa/gh-action-pypi-publish`, then verifies **`npm`/PyPI/GitHub Releases** parity. Register the workflow file **`release-package.yml`** as a **trusted publisher** for the `agentskeptic` project on [PyPI](https://pypi.org).
...
**v4 trust-lean OSS gates (same policy prelude):** [`scripts/assert-ci-json-parse.mjs`](scripts/assert-ci-json-parse.mjs); [`schemas/ci/v4-copy-invariants.manifest.json`](schemas/ci/v4-copy-invariants.manifest.json); [`schemas/ci/kernel-browser-open-patterns.json`](schemas/ci/kernel-browser-open-patterns.json); [`scripts/assert-artifacts-generated-not-tracked.mjs`](scripts/assert-artifacts-generated-not-tracked.mjs); [`scripts/assert-v4-copy-invariants.mjs`](scripts/assert-v4-copy-invariants.mjs); [`scripts/assert-no-browser-open-in-kernel.mjs`](scripts/assert-no-browser-open-in-kernel.mjs); [`scripts/assert-npm-footprint-improved.mjs`](scripts/assert-npm-footprint-improved.mjs); [`scripts/measure-npm-footprint.mjs`](scripts/measure-npm-footprint.mjs). **`config/first-five-minutes.json`** is regenerated from **`website/src/content/first-five-minutes.json`** by **`npm run build`** (**`scripts/materialize-first-five-minutes.mjs`**) and by **`npm run sync:public-product-anchors`** prelude via [`scripts/sync-website-ssot.mjs`](scripts/sync-website-ssot.mjs).

**Verification execution identity (merge receipts, Docker replay, live contract parity):** [`docs/verification-execution-ssot.md`](docs/verification-execution-ssot.md).

**Post-PyPI distribution verify:** the **`release-package.yml`** **`pypi`** job ends with **`Verify distribution`**, which runs `node scripts/verify-release-distribution.mjs` with `EXPECTED_VERSION` (from the tag) and `GH_TOKEN` (`secrets.GITHUB_TOKEN`) so **npm**, **PyPI**, and **GitHub Releases** all report the same version (fixed retry policy in the script). The working tree copy of OpenAPI under `website/public/` is **not** committed (gitignored); the integrity check targets only `schemas/openapi-commercial-v1.yaml` at the repo root. For local runs of the verify script, `gh` must be authenticated (e.g. `gh auth login`).
