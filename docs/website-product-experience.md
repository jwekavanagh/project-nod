...
- **`/integrate`:** [`website/src/app/integrate/page.tsx`](../website/src/app/integrate/page.tsx) is **pack-led only**: one `<pre>` with the crossing command from [`config/marketing.json`](../config/marketing.json) (`integratePage.packLedCommand`), a short requirements list, a proof line, and **two** GitHub links to normative runbooks. No `<details>`, no second command block, no long on-page runbook. Deeper steps stay in [`docs/first-run-integration.md`](first-run-integration.md) and related GitHub paths.
- **LangGraph reference boundaries:** [`docs/langgraph-reference-boundaries-ssot.md`](langgraph-reference-boundaries-ssot.md#langgraph-reference-documentation-boundaries) owns the full matrix; linked integrator docs must not duplicate that table shape here.
...
1. **`/`** — Homepage `<main>` stream: **Hero** (`productCopy` wires `heroTitle` to `<h1>`, `heroOutcome` + `heroMechanism` lede lines from [`config/marketing.json`](../config/marketing.json), a muted `guaranteeFootnote` with read-only / non-causal GitHub link, primary **`#try-it`** CTA, secondary **Product brief** CTA, terminal box with **VERDICT: FAILED** + `HeroTerminalHighlighted` + `shareableTerminalFailureExcerpt`, **Try it** embedded as `TryItSection` `variant="heroEmbedded"`); **What this catches**; **When the database tells a different story** (`homeStakes`); **How it works**; **Fit and limits**; **Closing**; **Commercial**. **`tryIt.runButton`** is the distinct POST label inside Try it. **`homepage-causality-invariant`** still governs causality wording. Hero `data-testid="home-hero-cta-row"`: primary **`#try-it`** **`btn`**, secondary acquisition **`btn secondary`**; **`/pricing` is not in the hero row**.
...
3. **`/pricing`** — hero (`pricingHero`), link to **`/guides/buyer-commercial-boundary`**, plan-choice subtitle (`pricingHero.subtitle`), **`/compare`** lead after the hero recap, **`pricingHeroExample`**, **`pricingRiskReassurance`**, then the server-rendered `<ul aria-label="Commercial terms">` list asserted by **`pricing-commercial-terms-html`** (each item’s first child is a single `<strong>` lead, normative `body` lines unchanged), then the trust band (`pricingTrustBandBeforeGrid`), then the client grid (`PricingClient`). The Individual card shows `data-testid="pricing-entry-paid-pill"`; the recommended Team card keeps `data-testid="pricing-recommended-pill"`.
...
When you change **`config/marketing.json`**, **`productCopy.ts`**, or any route markup covered by the marketing Vitest suite, run **`npm run verify:web-marketing-copy`** from the repository root as the single gate before merging.
...
- **Marketing / acquisition:** [`config/marketing.json`](../config/marketing.json) drives the README **`discovery-readme-title`** and discovery fold (sync-written), extended **`/llms.txt`**, and site hero copy. **Discovery routing and merge-gate metadata:** [`docs/discovery-surfaces.md`](../docs/discovery-surfaces.md). **Homepage `/`** uses [`website/src/lib/marketing.ts`](../website/src/lib/marketing.ts) plus [`website/src/content/productCopy.ts`](../website/src/content/productCopy.ts) for section bodies. Primary nav in [`website/src/app/SiteHeader.tsx`](../website/src/app/SiteHeader.tsx) maps `buildSiteHeaderPrimaryLinks` (**Product brief**, **Get started** `/integrate`, **Try** `/#try-it`, **Learn** `/guides`, **Problems** `/problems`, **Compare** `/compare`, **Pricing**, external **CLI**), then auth. CLI **`cliFollowupLines`** footer uses sync-generated **`src/publicDistribution.generated.ts`**. Validation: [`scripts/validate-marketing.cjs`](../scripts/validate-marketing.cjs) (not a large JSON Schema file). Do not hand-edit prose inside README sync markers; change JSON and run **`npm run emit-primary-marketing`** (or **`sync:public-product-anchors`**).
- **Generated (gitignored, do not hand-edit):** [`website/public/llms.txt`](../website/public/llms.txt) and [`website/public/openapi-commercial-v1.yaml`](../website/public/openapi-commercial-v1.yaml) — written by [`scripts/emit-primary-marketing.cjs`](../scripts/emit-primary-marketing.cjs). **`website` `prebuild`** runs **`npm run sync:public-product-anchors`** (alias for **`emit-primary-marketing`**) from the repo root so these exist before `next build`.
- **Committed static asset:** [`website/public/og.png`](../website/public/og.png) — Open Graph / Twitter preview image; regenerate with `node website/scripts/export-og-from-lockup.cjs` after changing [`mark.png`](../website/public/brand/mark.png) (see [`docs/brand-system-ssot.md`](brand-system-ssot.md)).
