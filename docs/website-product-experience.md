...
1. **`/`** — Homepage `<main>` text order is: `heroTitle`, then `homepageDecisionFraming`, then `heroSubtitle`, then hero CTAs, then the trust strip from `buildHomeTrustStripLinks`, then `howItWorks` through `commercialSurface`; `homepageHero` why/what/when is not part of that `<main>` text stream. Causality and mechanism limits are covered by **`homepage-causality-invariant`**. The hero `data-testid="home-hero-cta-row"` has **exactly two** controls: primary **`Link`** to **`discoveryAcquisition.slug`** (canonical acquisition narrative) with **`btn`** styling, then secondary **Try it** anchor to **`#try-it`** with **`btn secondary`**; **`/pricing` is not in the hero row** (pricing remains in the site header). That default puts the citeable “database truth vs traces” story one click from first paint for evaluators who land on `/`.
