...
- **`/integrate`:** [`website/src/app/integrate/page.tsx`](../website/src/app/integrate/page.tsx) renders **static** activation copy from [`website/src/content/productCopy.ts`](../website/src/content/productCopy.ts) (`integrateActivation`) and [`website/src/content/siteMetadata.ts`](../website/src/content/siteMetadata.ts) (`integrate.*`). The single command block uses **`data-testid="integrator-activation-commands"`** only; there is **no** LangGraph README CTA on this route and **no** `<details>` / markdown embed. Deep integrator commands and prose remain authoritative in [`docs/partner-quickstart-commands.md`](partner-quickstart-commands.md) and [`docs/first-run-integration.md`](first-run-integration.md). **Build-embedded** strings in [`website/src/generated/integratorDocsEmbedded.ts`](../website/src/generated/integratorDocsEmbedded.ts) (from [`scripts/sync-integrator-docs-embedded.mjs`](../scripts/sync-integrator-docs-embedded.mjs), **`website` `prebuild`**) are still generated for CI parity tests; the `/integrate` route does not import them.
- **LangGraph reference boundaries:** [`docs/verification-product-ssot.md`](verification-product-ssot.md#langgraph-reference-documentation-boundaries) owns the full matrix; linked integrator docs must not duplicate that table shape here.
...
1. **`/`** — Homepage `<main>` stream: **Hero** (`heroTitle`, `homepageDecisionFraming`, `heroSubtitle`, `productCopy.homeMomentLine`, demo-first CTAs, terminal box with **VERDICT: FAILED** + `HeroTerminalHighlighted` + `shareableTerminalFailureExcerpt`, **Try it** embedded as `TryItSection` `variant="heroEmbedded"`—`div`+`role="region"`+`id="try-it"`); **What this catches** (bullets + repeat CTA only); **When traces lie** (`homeStakes` + tagline); **How it works** (three steps + example/acquisition/security links + repeat CTA); **Fit and limits** (`mechanism.notObservability` at bottom); **Closing** (`homeClosing`: “Stop trusting traces alone”, CTA, then `buildHomeTrustStripLinks` with `data-testid="home-trust-strip-*"` for R3); **Commercial** (paid copy). Primary demo label: `HOME_DEMO_PRIMARY_CTA_LABEL` / `tryIt.runButton` / `homeHeroCtaLabels.demo` (same string). `homepageHero` why/what/when is not in that `<main>` stream. **`homepage-causality-invariant`** still governs causality wording. Hero `data-testid="home-hero-cta-row"`: primary **`#try-it`** **`btn`**, secondary acquisition **`btn secondary`**; **`/pricing` is not in the hero row**.
...
3. **`/pricing`** — outcome-first hero recap (`productCopy.pricingRecap`), plan-choice guide (`pricingPlanChoiceGuide`), then the server-rendered `<ul aria-label="Commercial terms">` list asserted by **`pricing-commercial-terms-html`** (each item’s first child is a single `<strong>` lead, normative `body` lines unchanged), then the trust band (`pricingTrustBandBeforeGrid`), then the client grid (`PricingClient`). The Individual card shows `data-testid="pricing-entry-paid-pill"`; the recommended Team card keeps `data-testid="pricing-recommended-pill"`.
...
- **npm registry fields:** Root [`package.json`](../package.json) **`description`** is written by sync from **`config/discovery-acquisition.json` → `pageMetadata.description`** (pain-led registry/unfurl copy). **`keywords`**, **`repository`**, **`bugs`** are written from [`config/public-product-anchors.json`](../config/public-product-anchors.json). **`homepage`** is written by sync as **`normalize(productionCanonicalOrigin)`** plus **`discovery.slug`** from [`config/discovery-acquisition.json`](../config/discovery-acquisition.json) (the canonical acquisition URL, not the bare site origin). **`identityOneLiner`** in anchors remains the precise line for OpenAPI / README anchor list / `llms.txt` Summary — not the npm `description`. Run **`npm run sync:public-product-anchors`** after editing either JSON file.
