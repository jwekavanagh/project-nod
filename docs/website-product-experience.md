...
1. **`/`** — Homepage `<main>` stream: **Hero** (`heroTitle`, `homepageDecisionFraming`, `heroSubtitle`, `productCopy.homeMomentLine`, demo-first CTAs, failure terminal excerpt via `HeroTerminalHighlighted` + `shareableTerminalFailureExcerpt`, **Try it** embedded under the same hero `<section>` as `TryItSection` `variant="heroEmbedded"`—`div`+`role="region"`+`id="try-it"`); then **What this catches** (`homeWhatCatches` bullets + repeated demo CTA + `buildHomeTrustStripLinks` rows keeping `data-testid="home-trust-strip-*"` for R3); **When traces lie** (`homeStakes`); **How it works** (three tight steps + outbound links + repeat CTA); **Fit and limits** (includes `mechanism.notObservability` at the bottom); **Commercial** (repeat CTA then paid copy). `homepageHero` why/what/when is not in that `<main>` stream. **`homepage-causality-invariant`** still governs causality wording. Hero `data-testid="home-hero-cta-row"`: primary **`#try-it`** **`btn`**, secondary acquisition **`btn secondary`**; **`/pricing` is not in the hero row**.
