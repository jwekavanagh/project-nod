...
**Shipped vs parsed-only SQL URLs:**

- **Shipped today:** **`sqlite`** (filesystem path), **`postgres`**, **`mysql`** — parameterized read-only connectors open normally.
- **Parsed only (`bigquery://`, `sqlserver://` / `mssql://`):** These schemes deserialize to `VerificationDatabase` for forward compatibility, but **verification does not yet ship matching SQL connectors** (`VERIFICATION_CONNECTOR_NOT_SHIPPED` on open). Do not pass production BigQuery/SQL Server URLs expecting a verdict until connectors exist (use a Postgres/MySQL-readable mirror or a row export). A future BigQuery connector would still **`RELATIONAL_UNSUPPORTED_DIALECT`** at resolve time (`sql_relational` off that dialect—see relational rules elsewhere).
...
| SQL Server | **Connector not shipped:** `sqlserver://` / `mssql://` URLs parse only (see §2). |
| BigQuery | **Connector not shipped:** `bigquery://` URLs parse only (see §2). |
