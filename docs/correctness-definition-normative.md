...
`correctnessDefinition` is **descriptive**: it states what must hold for verification to succeed. It does **not** instruct the engine to mutate data or run new SQL. Remediation alignment remains hints for humans and downstream policy ([`docs/agentskeptic.md`](agentskeptic.md)).
