...
- Server-rendered page using **`VerificationReportView`**. For **Outcome Certificate v3** envelopes (`schemaVersion` **3** + **`certificate`**), the page shows **in order**: (1) intro copy, (2) a short **authority note** (structured summary is a view over the certificate; **Outcome Certificate JSON** is the integration SSOT; **canonical human report** is the full audit artifact), (3) a **decision summary** derived **only from structured certificate fields**—not by parsing the canonical human report string. The summary includes verification verdict, headline, canonical primary reason, next action, explicit determinacy (`determinate mismatch` vs `unknown due to evidence blocker` vs verified match), and bounded coverage lists for checked / not-checked / missing-input evidence when present, (4) optional **LangGraph checkpoint trust** panel when `runKind` is **`contract_sql_langgraph_checkpoint_trust`**, (5) **Canonical human report** (`human_text` / `certificate.humanReport` bytes as stored), (6) **Machine JSON (integration SSOT)** (pretty-printed certificate or legacy payload). **Legacy** envelopes (`schemaVersion` **1**) show a notice that the structured decision summary is for v3 shares only; malformed v3 payloads show a certificate-unavailable notice.
