...
**Remediation decision SSOT:** `deriveRemediationDecisionFromWorkflowResult` and `deriveRemediationDecisionFromQuickReport` in [`src/actionableFailure.ts`](../src/actionableFailure.ts) produce **`RemediationDecision`** (`actionableFailure`, ordered summary **`nextActions`**, legacy **`rerunReadiness`**, structured **`remediationItems[]`**, and primary **`rerunPath`**). Certificate builders pass that decision into **`buildEvidenceCompleteness*`** and **`buildFailureSpine*`** so **`evidenceCompleteness.nextActions[0].id`** aligns with **`failureSpine.actionableFailure.recommendedAction`**, while **`evidenceCompleteness.remediationItems[]`** is the complete check-by-check convergence surface. New failed certificates must emit **`remediationItems[]`** and **`rerunPath`**; the schema keeps them optional so old artifacts remain readable. **`CONNECTOR_ERROR`** uses **`improve_read_connectivity`** with read-only retry automation; the human **`failure_spine`** may append the **`automation_boundary:`** line (`AUTOMATION_BOUNDARY_CONNECTOR` in [`src/remediationMessage.ts`](../src/remediationMessage.ts)).
...
**Advanced — local operator flow:** [`local-feedback-loop.md`](local-feedback-loop.md) (`agentskeptic loop`: normalized trust verdict + auto-compare + local run history). **Quick Verify (zero-config inference)** is specified in [`quick-verify-normative.md`](quick-verify-normative.md). **Activate / bootstrap pack** (`agentskeptic activate`, legacy `agentskeptic bootstrap`) is specified in [`bootstrap-pack-normative.md`](bootstrap-pack-normative.md). Product narrative, audiences, and documentation ownership for the wedge live in [`verification-product.md`](verification-product.md). This document remains the SSOT for **Advanced verification** (NDJSON events, `tools.json` registry, `WorkflowResult`) and compatibility commands (`quick`, `crossing`, `verify-integrator-owned`). **Compatibility:** bare positional **`agentskeptic --workflow-id … --events … --registry … --db …`** (same kernel as `check` without the primary-path stderr prefix).
...
**Distribution footer (batch verify + quick):** After **`humanReport`**, the CLI appends several stderr lines from **`formatDistributionFooter()`** in **`src/publicDistribution.generated.ts`** (generated by **`npm run emit-primary-marketing`** from **`config/marketing.json`** `cliFollowupLines`): canonical acquisition URL (`productionCanonicalOrigin` + acquisition slug), **`/integrate`**, site origin reminder, then a final line pointing at the public distribution SSOT on GitHub. With **`--no-human-report`**, batch verify omits the report and footer on stderr; **OSS** may still print the single telemetry status line described above. Exit **3** behavior is unchanged.
...
- **Exit 3:** **stdout** empty; **stderr** ends with one JSON line with **`kind`:** **`execution_truth_layer_error`** (see [CLI operational errors](#cli-operational-errors)). On **OSS**, an optional telemetry status line may appear before that JSON line.

There is **no** separate CI-only report format. Integrators should parse **stdout** for verdicts **0–2** and **stderr** for exit **3** (use the **last** JSON line on stderr when optional OSS telemetry text may precede the envelope), not the human report text when **`--no-human-report`** is used.
...
**`--no-human-report`:** For the cases below that verify exits **0** and **1**, the CLI **must** be invoked with **`--no-human-report`** so **stderr** contains **no human report** (and **OSS** may still emit only the optional telemetry status line). Automation should not assume stderr is byte-empty.
...
| **stderr** | Empty, or **OSS** telemetry status line only (see **I/O order (CLI — verdict paths 0/1/2)** above) |
...
| **stderr** | Empty, or **OSS** telemetry status line only |
...
| Exit code | **3** |
| **stdout** | Empty |
| **stderr** | Final line: JSON with **`schemaVersion`** **2**, **`kind`** **`execution_truth_layer_error`**, **`code`** **`CLI_USAGE`**, **`message`** non-empty string length ≤ **2048**, required **`failureDiagnosis`** (`summary`, **`primaryOrigin`**, **`confidence`**, **`evidence`**, **`actionableFailure`**) — see [`schemas/cli-error-envelope.schema.json`](../schemas/cli-error-envelope.schema.json). **OSS:** optional telemetry line may precede. |

**Enforcement:** **`test/ci-workflow-truth-postgres-contract.test.mjs`** implements these cases (including relational **`wf_rel_pg`** and operational **`CLI_USAGE`**); **`npm run test:workflow-truth-contract`** runs that file alone. Ordered verification for **`npm test`**, **`npm run test:ci`**, and **`npm run verification:truth`** is defined in [`docs/testing.md`](testing.md) and implemented by **`scripts/verification-truth.mjs`**. GitHub Actions runs **`npm run verification:truth`** in the **`verification`** job after **`npm ci`**.

<!-- ci:outcome-certificate-normative-prose:end -->

### CLI operational errors

When the CLI exits **3**, **stderr** contains **one** UTF-8 line that is a JSON object with (preceded only by optional **OSS** telemetry status text):
...
Primary remediation tokens are chosen from the same **`failureAnalysis.evidence`** walk and primary-code selection rules as category and severity: **`deriveActionableCategory`** and **`deriveActionableFailureWorkflow`** in **`actionableFailure.ts`** share that ordering so a single failure analysis cannot **contradict** itself (for example, a “bad input” category with a downstream-only remediation hint). Check-by-check convergence lives in **`evidenceCompleteness.remediationItems[]`**: every failed run-level issue, event-sequence issue, run-context blocker, failed step, failed effect, quick-ingest blocker, and non-verified quick unit gets its own action, expected state, automation class, human-review block, and conditional rerun path. Operational CLI failures take **`recommendedAction`** and **`automationSafe`** from **`OPERATIONAL_DISPOSITION`** rows keyed by the operational code.
...
These fields are **verification** metadata for **external** consumers: they describe posture and intent, not commands. Mixing automated **mutation** (writes, orchestration) with read-only verification in the same process would break trust boundaries. The library never spawns **subprocesses** or runs SQL **based on** **`recommendedAction`**, **`automationSafe`**, or **`remediationItems[].automation`**. Automation class **`read_only_retry`** means only retrying read-only verification with the same inputs after the read-only prerequisite is restored; **`input_regeneration_candidate`**, **`human_write_required`**, and **`never_auto_mutate`** require external action or judgment before rerun.
...
- **Verification** — see **[`docs/testing.md`](testing.md)**. **`npm test`**, **`npm run test:ci`**, and **`npm run verification:truth`** are the same command: full regeneration gate, Postgres-backed distribution checks, and journey suite. Requires Postgres 16+ fixture URLs (**`POSTGRES_ADMIN_URL`**, **`POSTGRES_VERIFICATION_URL`**, **`DATABASE_URL`**, **`TELEMETRY_DATABASE_URL`**); CI sets them in [`.github/workflows/ci.yml`](../.github/workflows/ci.yml). **`npm install` does not run `tsc`** (no **`prepare`**).
