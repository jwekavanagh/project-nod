...
**Distribution footer (batch verify + quick):** After **`humanReport`**, the CLI appends several stderr lines from **`formatDistributionFooter()`** in **`src/publicDistribution.generated.ts`** (generated by **`npm run emit-primary-marketing`** from **`config/primary-marketing.json`** `cliFollowupLines`): canonical acquisition URL (`productionCanonicalOrigin` + acquisition slug), **`/integrate`**, site origin reminder, then a final line pointing at the public distribution SSOT on GitHub. With **`--no-human-report`**, batch verify leaves stderr empty (no report and no footer). Exit **3** behavior is unchanged.
