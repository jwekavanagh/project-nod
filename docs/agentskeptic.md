...
**Remediation decision SSOT:** `deriveRemediationDecisionFromWorkflowResult` and `deriveRemediationDecisionFromQuickReport` in [`src/actionableFailure.ts`](../src/actionableFailure.ts) produce **`RemediationDecision`** (`actionableFailure`, ordered **`nextActions`**, **`rerunReadiness`**). Certificate builders pass that decision into **`buildEvidenceCompleteness*`** and **`buildFailureSpine*`** so **`evidenceCompleteness.nextActions[0].id`** aligns with **`failureSpine.actionableFailure.recommendedAction`**. Optional **`evidenceCompleteness.rerunReadiness`** and optional **`correctnessDefinition`** on the certificate (still **v3**, no v4) duplicate truth when present for attestation. **`CONNECTOR_ERROR`** uses **`improve_read_connectivity`** with **`automationSafe: true`**; the human **`failure_spine`** may append the **`automation_boundary:`** line (`AUTOMATION_BOUNDARY_CONNECTOR` in [`src/remediationMessage.ts`](../src/remediationMessage.ts)).
