...
- **Discovery assets (`/llms.txt`, `/openapi-commercial-v1.yaml`):** Served by App Router routes reading [`website/public/`](../website/public/) (see [`website/src/lib/publicDiscoveryAssetResponse.ts`](../website/src/lib/publicDiscoveryAssetResponse.ts)). If files are missing after a broken deploy, responses use a **generic** `404` body (`Not Found`) — not operational sync hints — so passive scanners do not flag “application error disclosure.” **`npm run sync:public-product-anchors`** (repo root) must succeed in CI/build so [`scripts/assert-discovery-public-files.mjs`](../scripts/assert-discovery-public-files.mjs) confirms both files exist after sync.
